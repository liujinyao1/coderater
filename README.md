好的，为你准备一份详细的 README 文档和 API 接口说明，方便你的组员理解和协作。
...
欢迎克隆项目，并根据分工进行开发。请遵循以下协作流程：

1.  从 `main` (或 `develop`) 分支创建新的特性分支 (e.g., `feature/user-auth`, `feature/improve-scoring`)。
2.  在特性分支上进行开发和提交。
3.  定期 `pull` 主分支的最新代码到你的特性分支，解决冲突。
4.  完成特性开发并通过本地测试后，发起 Pull Request (PR) 到主分支。
5.  至少需要一名其他组员 Code Review 通过后，方可合并 PR。
6.  保持良好的 Git 提交信息规范。
